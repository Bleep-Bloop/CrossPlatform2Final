...
public class ParabolaController : MonoBehaviour
{
    ...
    // Middle point of the Parabola -Taisen awful programming here
    ...
    // I THINK THIS JUST RESETS THE GIZMO
    /// <summary>
    /// Called after the cannon retargets the player. Resets the mid point so the projectile is not arcing.
    /// </summary>
    public void ResetParabolaMidPoint()
    {

        float xPosition = ParabolaRoot.transform.position.x + (EndPoint.transform.position.x - ParabolaRoot.transform.position.x); 
        float yPosition = ParabolaRoot.transform.position.y + (EndPoint.transform.position.y - ParabolaRoot.transform.position.y); 
        float zPosition = ParabolaRoot.transform.position.z + (EndPoint.transform.position.z - ParabolaRoot.transform.position.z); 

        HeightPoint.transform.position = new Vector3(xPosition, yPosition + 5, zPosition);
